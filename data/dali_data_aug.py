...
def ExternalSourceTrainPipeline(batch_size, num_threads, device_id, external_data, train_width, train_height, top_crop, normalize_image_scale=False, nscale_w=None, nscale_h=None):
    """Original pipeline (geometric augmentations only, no flip/color jitter)."""
...
def ExternalSourceTrainPipelineAug(batch_size, num_threads, device_id, external_data, train_width, train_height, top_crop, normalize_image_scale=False, nscale_w=None, nscale_h=None):
    """Augmented pipeline: adds horizontal flip, brightness/contrast, and color jitter (hue/saturation).

    Horizontal flip is applied via crop_mirror_normalize's mirror parameter so that
    the spatial crop and flip happen atomically. The flip coin is emitted as a 4th
    output so TrainCollectAug can mirror lane x-coordinates in Python.

    Brightness/contrast and HSV jitter are image-only and do not affect labels.
    """
    pipe = Pipeline(batch_size, num_threads, device_id)
    with pipe:
        jpegs, seg_images, labels = fn.external_source(source=external_data, num_outputs=3)
        ...
        # --- Geometric augmentations (applied identically to image, seg, and labels) ---
        mt = fn.transforms.scale(scale=fn.random.uniform(range=(0.8, 1.2), shape=[2]), center=center)
        mt = fn.transforms.rotation(mt, angle=fn.random.uniform(range=(-6, 6)), center=center)
        ...
        # --- Horizontal flip (coin emitted so Python side can mirror coordinates) ---
        ...
        # --- Color jitter: Hue & Saturation (image only) ---
        images = fn.hsv(
            images,
            hue=fn.random.uniform(range=(-18.0, 18.0)),
            saturation=fn.random.uniform(range=(0.7, 1.3)),
        )

        # --- Fog effect: blend toward white (image only, ~30% probability) ---
        # Simulates haze/fog by mixing image with a white layer.
        # blend_alpha=1 means no fog; lower values produce denser fog.
        fog_coin = fn.random.coin_flip(probability=0.3)
        fog_alpha = fn.random.uniform(range=(0.5, 0.85))
        fog_coin_f = fn.cast(fog_coin, dtype=types.FLOAT)
        blend_alpha = fog_coin_f * fog_alpha + (1.0 - fog_coin_f)
        images = images * blend_alpha + 255.0 * (1.0 - blend_alpha)

        # --- Crop + normalize + horizontal flip (atomic via mirror=flip_coin) ---
        ...
        pipe.set_outputs(images, seg_images, labels, flip_coin)
    return pipe
...
class TrainCollect:
    """Original TrainCollect (no flip/color-jitter augmentation)."""
    def __init__(self, batch_size, num_threads, data_root, list_path, shard_id, num_shards,
        ...
        eii = LaneExternalIterator(data_root, list_path, batch_size=batch_size, shard_id=shard_id,
                                   num_shards=num_shards, dataset_name=dataset_name)
        ...
        if dataset_name == 'CurveLanes':
            pipe = ExternalSourceTrainPipeline(batch_size, num_threads, shard_id, eii,
                                               train_width, train_height, top_crop,
                                               normalize_image_scale=True, nscale_w=2560, nscale_h=1440)
        else:
            pipe = ExternalSourceTrainPipeline(batch_size, num_threads, shard_id, eii,
                                               train_width, train_height, top_crop)

        self.pii = DALIGenericIterator(pipe, output_map=['images', 'seg_images', 'points'],
    ...
    def __next__(self):
        ...
        return {'images': images, 'seg_images': seg_images,
    ...
    def _extend(self, coords):
        ...
        for i in range(n):
            for j in range(num_lanes):
                ...
                if num_valid_pts < 6:
                    continue
                p = np.polyfit(coords_axis[valid][num_valid_pts // 2:],
                               lane[valid][num_valid_pts // 2:], deg=1)
...
class TrainCollectAug:
    """TrainCollect with horizontal flip + brightness/contrast + color jitter (hue/saturation).

    Uses ExternalSourceTrainPipelineAug which emits a 4th 'flip' output.
    When a sample is flipped, x-coordinates are mirrored (x -> W - x) and
    lane order is reversed so left/right lanes remain consistent with the image.
    """
    def __init__(self, batch_size, num_threads, data_root, list_path, shard_id, num_shards,
                 row_anchor, col_anchor, train_width, train_height, num_cell_row, num_cell_col,
                 dataset_name, top_crop):
        ...
        if dataset_name == 'CurveLanes':
            pipe = ExternalSourceTrainPipelineAug(batch_size, num_threads, shard_id, eii,
                                                  train_width, train_height, top_crop,
                                                  normalize_image_scale=True, nscale_w=2560, nscale_h=1440)
        else:
            pipe = ExternalSourceTrainPipelineAug(batch_size, num_threads, shard_id, eii,
        ...
        self.pii = DALIGenericIterator(pipe,
                                       output_map=['images', 'seg_images', 'points', 'flip'],
                                       last_batch_padded=True, last_batch_policy=LastBatchPolicy.PARTIAL)
        self.eii_n = eii.n
        self.batch_size = batch_size
    ...
    def __next__(self):
        ...
        seg_images = data[0]['seg_images']
        points = data[0]['points']
        flip = data[0]['flip'].view(-1).bool()  # [B]

        # Mirror x-coordinates and reverse lane order for horizontally flipped samples.
        # points shape: [B, num_lanes, num_points, 2]  (dim -1: [x, y])
        if flip.any():
            pts = points[flip].clone()
            pts[..., 0] = self.original_image_width - pts[..., 0]
            points[flip] = pts.flip(dims=[1])
...
class TestCollect:
    def __init__(self, batch_size, num_threads, data_root, list_path, shard_id, num_shards):
        self.batch_size = batch_size
        eii = LaneExternalIterator(data_root, list_path, batch_size=batch_size, shard_id=shard_id,
                                   num_shards=num_shards, mode='test')
        pipe = ExternalSourceTestPipeline(batch_size, num_threads, shard_id, eii)
        self.pii = DALIGenericIterator(pipe, output_map=['images', 'names'],
                                       last_batch_padded=True, last_batch_policy=LastBatchPolicy.PARTIAL)
        self.eii_n = eii.n
